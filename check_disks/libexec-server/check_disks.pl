...
=head1 PLUGIN
    ...
    check_disk.pl : check disks on Unix, Linux or Windows.
...
=head1 CONTEXT
    ...
    Project: Nagios Disks Monitoring for Windows and Unix/Linux Servers.
	------------------------------------------------------------------------------------------
...
=head2 Without config file.

    All filesystems having same thresholds:
...
free)] [/var 1.5G (73% free)] [/tmp 989.7M (96% free)] [/home 1.8G (90% free)]
    ...
    Force thresholds to certain filesystems
...
=head2 Using a config file

    Syntax (check_disk.cfg): 
    ...
    Filesystem thresholds are read from this configuration file. Note that -f option will be ignored, if used.
...
=head1 HISTORY
    ...
    Revision 1.2 2014/07/08 haukur@hauxi.is
    o Output summarizes warnings
	o Refactor of code and text.
...
$ENV{'PATH'} = "/usr/lib64/nagios/plugins:/usr/lib6464/nagios/plugins:/usr/local/libexec:/usr/lib64exec:/usr/local/nagios/libexec";
...
foreach my $f (keys %checkdisks) {
    ...
    if($checkdisks{$f}->{pfree} < $checkdisks{$f}->{critical}) {
        $critical_disks .= " " . $f . " (" . $checkdisks{$f}->{pfree} . "% / ". byte2human($checkdisks{$f}->{free}) . " free )";
        $cmp_crit++;
    } 
    elsif ($checkdisks{$f}->{pfree} < $checkdisks{$f}->{warning}) {
        $warning_disks .= " " . $f . " (" . $checkdisks{$f}->{pfree} . "% / ". byte2human($checkdisks{$f}->{free}) . " free)";
        $cmp_warn++;
    } else {
        $ok_disks .= " " . $f ;
    }
