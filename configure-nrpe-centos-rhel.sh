...
yum install nrpe -y
yum install nagios-plugins-swap nagios-plugins-ntp nagios-plugins-load nagios-plugins-procs -y
...
echo "nrpe ALL = (root) NOPASSWD: /usr/lib64/nagios/plugins/check_yum" >> /etc/sudoers
